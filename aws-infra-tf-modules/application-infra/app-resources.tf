...
########################################################
resource "tls_private_key" "app_ssh_data" {
  algorithm = "RSA"
}

resource "aws_key_pair" "ssh_key" {
  key_name   = "app-admin-key"
  public_key = tls_private_key.app_ssh_data.public_key_openssh
...
#############################################################
resource "aws_launch_template" "app_node_lt" {
  ...
  iam_instance_profile {
    arn = aws_iam_instance_profile.application_node_profile.arn
  }
...
resource "aws_autoscaling_group" "app_node_asg" {
  depends_on = [aws_alb.app_node_alb]
...
resource "aws_autoscaling_attachment" "app_node_alb_att" {
  alb_target_group_arn   = aws_lb_target_group.app_target_group.arn
  autoscaling_group_name = aws_autoscaling_group.app_node_asg.name
}
...
resource "aws_lb_listener" "app_alb_listener" {
  load_balancer_arn = aws_alb.app_node_alb.arn
